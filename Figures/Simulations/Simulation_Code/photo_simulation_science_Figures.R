...
wd <- "/Users/loewingergc/Desktop/Research/sims_delays" # simulation_delays" # sim_delay2   #"/Users/loewingergc/Desktop/Research/science_simulates"
setwd(wd)
library(dplyr)
library(latex2exp)
library(kableExtra)
...
nVec <- c(4:7) # just compare subset
fixed_smooth <- TRUE
knots_div <- 2
indp <- c(FALSE) 
resid_var_subj <- TRUE
...
ls_power_se <- ls_avg_se <- ls_power <- ls_time <- ls_rmse_avg
...
for(n in nVec){
  for(rwdLen in rwdLen_vec){
    ...
    if(file.exists(flNm)){
      ...
      avg_se <- data.frame("FLMM" = sd(d$fLME_reward_avgCIs,na.rm=TRUE)/sqrt(sum(!is.na(d$fLME_reward_avgCIs))),
                                "Perm" = sd(d$perm_CI_incl_reward,na.rm=TRUE)/sqrt(sum(!is.na(d$perm_CI_incl_reward))))

      names(avg_se) <- gsub(x = names(avg_se), pattern = "\\.", replacement = "-")
      ...
      power_se <- data.frame("FLMM" = sd(d$fLME_avgSignif,na.rm=TRUE)/sqrt(sum(!is.na(d$fLME_avgSignif))),
                              "Perm" = sd(d$perm_auc_signif,na.rm=TRUE)/sqrt(sum(!is.na(d$perm_auc_signif))),
                              "Perm_aoS_half" = sd(d$perm_avgSignif,na.rm=TRUE)/sqrt(sum(!is.na(d$perm_avgSignif))),
                              "Perm_aoS1" = sd(d$perm_avgSignif_1,na.rm=TRUE)/sqrt(sum(!is.na(d$perm_avgSignif_1)))) 
      
      names(power_se) <- gsub(x = names(power_se), pattern = "\\.", replacement = "-")
...
###########################################
# set colors
levs <- c("FLMM", "LMM", "t-test", "Perm")
myColors <- setNames( c("#ca0020", "#0868ac", "darkgray", "#E69F00"), levs) # , 
...
grp_incl <- c("FLMM", "Perm") # include these methods
###########################################
joint_ci = 
  dat %>% tibble %>%  
  ...
  geom_line( lwd = 2, aes(colour = key) ) + 
  geom_hline(yintercept=0.95, linetype="dashed", color = "black", size = rel(1), alpha = 0.7) + 
  geom_point(aes(colour = key), size = 2) + 
...
grp_incl <- c("FLMM", "LMM", "Perm", "t-test") # include these methods
###########################################
...
dat_se <- do.call(rbind, ls_avg_se) # standard error of the mean for functional methods
avg_dat_se <- inner_join(dat_se, avg_dat, by = c("key", "n"))
avg_dat_se$se_up <- avg_dat_se$value.y + 1.96*avg_dat_se$value.x
avg_dat_se$se_low <- avg_dat_se$value.y - 1.96*avg_dat_se$value.x
avg_dat_se$value <- avg_dat_se$value.y

plt = 
  ggplot(NULL, aes(x = as.factor(n), y = value)) + 
  geom_line(data = avg_dat, aes(y = value, x = as.factor(n), colour = key, group = key), lwd = 2) + 
  geom_point(data = avg_dat, aes(y = value, x = as.factor(n), colour = key, group = key), size = 2) + 
  geom_ribbon(data = avg_dat_se, aes(ymin=se_low, ymax=se_up, fill = key, group = key), alpha=0.3) + 
  geom_hline(yintercept=0.95,
             linetype="dashed",
             color = "black",
             size = rel(1),
             alpha = 0.7) + #
  ylab("Point-wise 95% CI Coverage" ) + 
  xlab( "Sample Size" ) + 
...
# standard error of the mean
dat_se <- do.call(rbind, ls_power_se) # standard error of the mean for functional methods
avg_dat_se <- inner_join(dat_se, avg_dat, by = c("key", "n"))
avg_dat_se$se_up <- avg_dat_se$value.y + 1.96*avg_dat_se$value.x
avg_dat_se$se_low <- avg_dat_se$value.y - 1.96*avg_dat_se$value.x
avg_dat_se$value <- avg_dat_se$value.y
...
# set colors
levs <- c("FLMM", "pffr") 
myColors <- setNames( c("#ca0020", "#0868ac"), levs) # , 
...
plt_box = 
  ...
  xlab( "Sample Size" ) + 
  scale_fill_manual(values = myColors[!levs %in% c( "LMM", "t-test", "Perm 1/2")] ) +
