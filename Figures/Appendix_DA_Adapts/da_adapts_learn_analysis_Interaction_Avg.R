...
# Figure 8d 
##########################################################
# a) Trial types defined as “lick+” trials (purple) with at least one lick during the delay between cue 
# and reward and “lick-” trials (green) with no delay period licking. 
# b) Percent of total trials that are “lick+” (top) and reward collection latency (bottom), for best 4 (green) and worst 4 (purple) performing mice, as determined by their reward collection latency in trials 700–800. 
# c) NAc–DA signals in the second half of training (trials 400–800) lick- (green) and lick+ (purple) trials, for ACTR simulations (top) and worst (left) and best (right) top 4 performing mice in terms of reward collection latency in trials 700–800. d) The ratio of NAc–DA reward signals on lick- vs lick+ trials was correlated with the final reward collection latency (Pearson’s r = −0.71, p = 0.03). 
#*** d) The ratio of NAc–DA reward signals on lick- vs lick+ trials was correlated with the final reward collection latency (Pearson’s r = −0.71, p = 0.03)
...
source('~/Desktop/NIMH Research/git_repos/fast_univ_fda/code/plot_fReg.R')
...
dat2$lickState <- as.factor(dat2$lickState)
...
final_latency <- dat2 %>%
  dplyr::filter(trial_num <= 800,
                trial_num >= 700) %>%
  group_by(ids) %>%
  summarise(final_latency = mean(latency))

trials_incl <- seq(400, 800) # photometry data trials
...
dat3 <- dat2 %>%
  dplyr::filter(latency <= 1000,
                ...
                trialID == 0,
                stimState == 0,
                trial_num %in% trials_incl) %>%
  group_by(ids, lickState) %>%
  dplyr::select(-group, -seshID) %>%
  summarise_all(mean) # throws errors for IDs and non-numeric that can be ignored safely


dat3 <- left_join(dat3, final_latency, by = "ids") %>% as.data.frame()

dat3$lickState <- as.numeric(dat3$lickState)
dat3$final_latency <- as.numeric(scale(dat3$final_latency))



# only one random effect is identifiable here because of low numbers of observations
DA_early <- fui(formula = photometry ~ final_latency + lickState + final_latency * lickState +
                    (final_latency-1| ids), # does slightly better than random intercept model
                  data = dat3,  
                  family = "gaussian", 
                  silent = FALSE,
                  var = TRUE, 
...
for(prdtr in 1:nrow(fit_dat$betaHat)){
  plot.f4[[prdtr]] <- plot.FUI(r = prdtr, 
                               Hz = target_Hz, 
                               align = tm,
                               var_name = c("Mean Signal on Avg. Latency Lick- Trial", 
                                            "Final Latency", 
                                            "Lick State",
                                            "Final Latency x Lick State"),
                               y_scal_orig = 0.01) 
...
# save
setwd("~/Desktop/NIMH Research/Photometry/fLME_methods_paper/Figures/da_adapts_learn")
ggsave("lick+-_latency_interaction_Avg.pdf",
       plot = fig,
       width = 8,
       height = 8)
