...
pre_min_tm <- pre_reward_period_length #+ pre_lick_reward_period # pre_lick period -- REMOVED pre_lick_reward_period because of errors below
  ...
  for(s in session_min:session_max){
    ...
    if(length(fl_list) == 1){ 
      ...
      if(iri_length > 8){
        ...
        # center around licks
        idx <- t( sapply(reward_idx[trial_iri], function(x) seq(x - pre_samps, x + post_samps) ) ) # matrix of indices for trials
...
for(prdtr in 1:nrow(fit_dat$betaHat)){
  plot.f4[[prdtr]] <- plot.FUI(r = prdtr, 
                                 Hz = target_Hz, 
                                 align = tm,
                                 x_lab = "Time from Reward-Delivery (s)",
...
p2 <- 
  ...
  geom_ribbon(data = mean_dat0, aes(ymin=y_low, ymax=y_high, alpha=0.4)) + 
  geom_line(data = mean_dat0, colour = "black", size = 0.75) +  
...
mean_signal$betaHat[1,] <- mean_dat0$photometry
diag(mean_signal$betaHat.var[,,1]) <- mean_dat0$sem
...
plot.f4[[1]] <- plot.FUI(r = prdtr, 
                               Hz = target_Hz, 
                               align = tm,
                               x_lab = "Time from Reward-Delivery (s)",
                               ylim = c(-0.5, 5),
                               var_name = c("Mean Signal: Sessions 1-8", "Average Signal", "Trial Number Association")) + 
...
# save
setwd("/Users/loewingergc/Desktop/NIMH Research/Photometry/fLME_methods_paper/Figures/Science-Exp1/Final/Cue Aligned")
ggsave( "Cue-Aligned Average Signal.pdf",
        plot = fig,
        width = 12,
        height = 4)
