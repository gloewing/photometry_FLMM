# plot fLME models with random effects
...
plot.FUI_RE <- function(r, 
  ...
  p.CI <- ggplot() +
    ...
    geom_line(aes(x = s / Hz - align/Hz - 1/Hz, y = beta, color = "#E69F00"), #",#ca0020" "#0868ac", "darkgray", "#E69F00", "#525252",
  ...
  p.CI <- p.CI + 
    labs(x = "Time (s)", y = bquote(paste("Animal-Specific Slope: ", beta[.(r-1)], "(s) + ", gamma[i][","][.(r-1)], "(s)")), 
         title = var_name[r]) +
  ...
  p.CI <- p.CI +
    geom_line(aes(x = s / Hz - align/Hz - 1/Hz, y = beta), #",#ca0020" "#0868ac", "darkgray", "#E69F00", "#525252",
              data = beta.hat.plt, alpha = 1, size = 1, colour = "#E69F00")
